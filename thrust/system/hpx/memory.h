...
#include <thrust/system/hpx/detail/tag.h>
...
{
  typedef thrust::system::hpx::reference<Element> type;
...
#if 0
/*! \p hpx::tag is type representing Thrust's HPX backend system in C++'s type system.
 ...
 *
 *  \p hpx::tag is convertible to \p cpp::tag.
 */
struct tag { unspecified };
#endif

/*! \p pointer stores a pointer to an object allocated in memory available to the hpx system.
 ...
 *
 *  \p pointer can be created with the function \p hpx::malloc, or by explicitly calling its constructor
 *  with a raw pointer.
 *
 *  The raw pointer encapsulated by a \p pointer may be obtained by either its <tt>get</tt> member function
             ...
             >
{
  ...
  public:
    // note that hpx::pointer's member functions need __host__ __device__
     ...
     *          to \p thrust::system::hpx::tag and its element type shall be convertible to \p T.
     */
...
/*! \p reference is a wrapped reference to an object stored in memory available to the \p hpx system.
 *  \p reference is the type of the result of dereferencing a \p hpx::pointer.
 *
...
/*! \p hpx::allocator is the default allocator used by the \p hpx system's containers such as
 *  <tt>hpx::vector</tt> if no user-specified allocator is provided. \p hpx::allocator allocates
 *  (deallocates) storage with \p hpx::malloc (\p hpx::free).
 */
template<typename T>
  struct allocator
...
#include <thrust/system/hpx/detail/adl_targets.h>
